...
def train():
    ...
    # evaluation
    acc_train = accuracy(output[idx_train], labels[idx_train].to(device))   # TODO: train evaluation
    # loss function
    loss_train = F.nll_loss(output[idx_train], labels[idx_train].to(device))    # TODO: loss function
    loss_train.backward()
    optimizer.step()
...
def validate():
    ...
    with torch.no_grad():
        output = model(adj, features)
        loss_val = F.nll_loss(output[idx_val], labels[idx_val].to(device))    # TODO: loss function
        acc_val = accuracy(output[idx_val], labels[idx_val].to(device))   # TODO: val evaluation
...
def test():
    ...
    with torch.no_grad():
        output = model(adj, features)
        loss_test = F.nll_loss(output[idx_test], labels[idx_test].to(device))    # TODO: loss function
        acc_test = accuracy(output[idx_test], labels[idx_test].to(device))   # TODO: test evaluation
